...
const governancelink = '<br><a href="/lsapp/teams.php?teamname=Governance">Planning, Evaluation, and Governance </a>';
const leaderslink = '<br><a href="/lsapp/teams.php?teamname=Leaders">Corp. Learning - People Leaders </a>';
const employeeslink = '<br><a href="/lsapp/teams.php?teamname=Employees">Corp. Learning - All Employees </a>';
const operationslink = '<br><a href="/lsapp/teams.php?teamname=Operations">Operations and Technology </a>';

var txtbox = document.getElementById('namesearch');
txtbox.onkeyup = function(e) {
    ...
    var output = '<div class="peoplebox p-3 bg-light-subtle shadow-sm rounded-3">';
    data.forEach(function(item,index){
    if (
            (item.idir.search(regex) != -1) || 
            (item.name.search(regex) != -1) || 
            (item.team.search(regex) != -1) || 
            (item.title.search(regex) != -1)
        ) {
  
            output += '<div class="person bg-light-subtle mb-2 p-3 rounded-3" style="width: 320px;">';
            output += '<div class="person-name"><a href="/lsapp/person.php?idir=' + item.idir + '">' + item.name + '</a>';
            if(item.pronouns != 'Unspecified') output += ' ' + item.pronouns;
            output += '<a class="d-inline-block py-1 px-2" href="https://teams.microsoft.com/l/chat/0/0?users=' + item.email + '">';
            ...
            output += '</a>';
            output += '</div>';
            output += '<div class="person-title">' + item.title;
            if(item.team == 'Governance') output += governancelink;
            if(item.team == 'Leaders') output += leaderslink;
            if(item.team == 'Employees') output += employeeslink;
            if(item.team == 'Operations') output += operationslink;
    ...
    output += '</div>';
    document.getElementById('personresults').innerHTML = output;
