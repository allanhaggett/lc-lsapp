			...
			<div class="dropdown">
				...
				<?php foreach($courses as $course): ?>
					...
					<a class="dropdown-item" href="index.php?courseids=<?= $cids ?>,<?= h($course[0]) ?>" class="btn btn-sm btn-primary">
						<?= h($course[2]) ?>
						<span class="badge bg-light-subtle "><?= $course[4] ?></span>
					</a>
...
<!-- Ben's Updates - Only show the No Ship badge if the class is No Ship AND not a Webinar or eLearning -->
		<?php if($row[49] == 'No Ship' and $row[45] !== 'Webinar' and $row[45] !== 'eLearning'): ?>
			<span class="badge bg-light-subtle "><?= h($row[49]) ?></span>
...
<!-- End of Ben's Updates -->
		...
		<?php endforeach ?>
		<?php if(empty($row[14]) and $row[1] != 'Inactive'): ?>
		...
		<!--<td><?= h($row[47]) ?></td>-->
		<td class="enrolled text-center" style="background:rgba(0,0,0,.05)"> <!-- Enrolled -->
		    <?php if($row['18'] + $row['19'] < $row[11] && $row[1] != 'Inactive' && $row[4] != 'Dedicated'): ?> <!-- Enrolled & Reserved less than minimum --> 
		        <span class="badge text-bg-danger" title="Enrollment is currently below the set minimum"><?= h($row[18]) ?></span>
		    <?php elseif($row['18'] < $row[11] && $row['18'] + $row['19'] > $row[11] && $row[1] != 'Inactive' && $row[4] != 'Dedicated'): ?> <!-- Enrolled less than min, but enrolled & reserved is over min -->
		        <?php $underenrolledreserved = TRUE; ?>
		        <span class="badge text-bg-warning" title="Enrollment only meets the set minimum with reserved seats"><?= h($row[18]) ?></span>
            <?php else: ?>
		        <span class="badge bg-light-subtle "><?= h($row[18]) ?></span>
		    <?php endif ?>
		</td>
		<td class="text-center"> <!-- Reserved -->
		    <?php if($underenrolledreserved): ?>  
		        <span class="badge text-bg-warning" title="Enrollment only meets the set minimum with reserved seats"><?= h($row[19]) ?></span>
			    <?php $underenrolledreserved = FALSE; ?>
		    <?php else: ?>
			    <span class="badge bg-light-subtle "><?= h($row[19]) ?></span>
		    <?php endif ?>
		</td> 
		<td class="waitlist text-center"><span class="badge bg-light-subtle "><?= h($row[21]) ?></span></td> <!-- Waitlisted -->
        <td class="text-center"><span class="badge bg-light-subtle "><?= h($row[20]) ?></span></td> <!-- Pending -->
	</tr>
<?php endforeach ?>
</tbody>
</table>
