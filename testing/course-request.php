...
<div class="col">
<div class="form-group">
<label for="category">Category <span style="font-size: 10px;color:red">*REQUIRED</span></label><br>
<small>General area of learning available in the Learning System</small>
<div class="card">
<div class="card-body">
<?php $categories = getCategories() ?>
<?php foreach($categories as $cat): ?>
<div class="form-check">
	<input class="form-check-input" type="checkbox" name="Category[]" value="<?= $cat[1] ?>" id="<?= $cat[1] ?>">
...
$(document).ready(function(){
	...
	$('#CourseName').keyup(function () {
	  ...
	  if (len >= max) {
		$('#cnameCharNum').removeClass('alert-success').addClass('alert-danger').text(' YOU ARE OVER THE CHARACTER LIMIT!');
	  } else {
		var char = max - len;
	...
	$('#CourseShort').keyup(function () {
	  ...
	  var len = $(this).val().length;
	  if (len >= max) {
		$('#cnameshortCharNum').removeClass('alert-success').addClass('alert-danger').text(' YOU ARE OVER THE CHARACTER LIMIT!');
	  } else {
		var char = max - len;
	...
	$('#CourseDescription').keyup(function () {
	  ...
	  var len = $(this).val().length;
	  if (len > max) {
		$('#cdescChar').removeClass('alert-success').addClass('alert-danger').text(' YOU ARE OVER THE CHARACTER LIMIT!');
	  } else {
		var char = max - len;
