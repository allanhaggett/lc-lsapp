...
<a href="audits.php">All Audits</a> &bull; <a href="/lsapp/audit-update-form.php?auditid=<?= $audit->AuditID ?>">Edit</a>
...
<div>
	...
	<?php else: ?>
	<span class="badge bg-warning "><?= $audit->Status ?></span> 
	<?php endif ?>

	<?= $audit->resourceType ?> Audit</div>

	
	<?php if(!empty($audit->LSAppCourseid)): ?>
		<h1>
			<a href="/lsapp/course.php?courseid=<?= $audit->LSAppCourseid ?>">
...
<div><?= $Parsedown->text($audit->AlignOrganizationalGoals ?? '')  ?></div>
